...
-- Insert Loan Procedure: receives a signature; 
-- checks current USER exists,
-- and that there is a hold on the copy (signature) for the current user as of today, 
-- and turns that reservation into a loan; 
-- otherwise (no reservation), checks that the copy is available for two weeks, 
-- and that the current user has not reached the upper limit for loans (nor they are sanctioned); 
-- when loan is possible inserts a new loan row, else reports about the problem. 
...
CREATE OR REPLACE PACKAGE BODY foundicu AS
    ...
    ELSE
        ...
        -- if the copy is available, check if the user has not reached the upper limit for loans by counting his loans
            ...
            IF count_reservations < 3 THEN
                ...
                IF SYSDATE < ban_date THEN
                    -- if the user is sanctioned, raise error
                    dbms_output.put_line('Error. Current user is banned');
                    RETURN;
                ELSE
                    dbms_output.put_line('Current user is not banned');
                    -- if the user is not sanctioned, insert a new loan row
    ...
    EXCEPTION
        ...
        WHEN TOO_MANY_ROWS THEN
            dbms_output.put_line('Error. Multiple current users found with the same primary key.');
        WHEN OTHERS THEN
            dbms_output.put_line('Error. Unknown error occurred');
    END insert_loan;
